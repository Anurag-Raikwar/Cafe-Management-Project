...
export class ManageOrderComponent implements OnInit {
  ...
  setQuantity(value:any){
    ...
    if(temp>0){
      ...
      var temp = this.manageOrderForm.controls['quantity'].setValue('1');
      this.manageOrderForm.controls['total'].setValue(this.manageOrderForm.controls['quantity'].value * this.manageOrderForm.controls['price'].value)
    }
  }
  validateProductAdd(){
  ...
  add(){
    console.log("data of form",this.manageOrderForm.value)
    var formData = this.manageOrderForm.value;
    var productName = this.dataSource.find((e:{id:Number;})=>e.id == formData.product.id);

    if(productName === undefined){
      this.totalAmount = this.totalAmount + formData.total;
      this.dataSource.push({id:formData.product.id,name:formData.product.name,category:formData.category,price:formData.price,quantity:formData.quantity,total:formData.total})
